...
class PlainNetFileParser(Parser):
	#comment_symbol = None
	column_separator = '\t'
	self_loop = False
	directed = False
...
# header ?
	...
	def __init__(self):
		super(PlainNetFileParser, self).__init__()
	...
	def finalize_output(self):
		...
		for i in self.next_level_parser.output_data:
			if type(i) == list and len(i)==1:
				...
				if not i[0] in temp_nodes:
					temp_nodes[i[0]] = None
				if not i[1] in temp_nodes:
					temp_nodes[i[1]] = None
				if not self.self_loop and i[0] == i[1]:
					continue
				if i[1] in temp_edges and i[0] in temp_edges[i[1]]:
					continue
				if not i[0] in temp_edges:
					temp_edges[i[0]] = {}
				if not i[1] in temp_edges[i[0]]:
					temp_edges[i[0]][i[1]] = None
				if len(i) > 2:
					if temp_edges[i[0]][i[1]] == None or temp_edges[i[0]][i[1]] < float(i[2]):
						temp_edges[i[0]][i[1]] = float(i[2])
			else:
				print "??" + str(i)
		self.output_data = []
